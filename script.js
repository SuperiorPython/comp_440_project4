...
/*
    Pocket Clash - Commit 2

    This commit adds:
    - Move data model
    - Pokemon data model
    - Simplified type chart
    - Eight available Pokemon
    - Dynamic battle screen rendering

    Battle turns, damage, fainting, and win/loss logic
...
class Pokemon {
    ...
    getHPPercentage() {
        return Math.max(
            0,
            Math.min(100, (this.currentHP / this.maxHP) * 100)
        );
    }
...
/*
    Required six type relationships:
    ...
    Reverse relationships are not very effective.
    All other relationships are neutral.
*/
...
const playerHPTrack = document.getElementById("player-hp-track");
...
const opponentHPTrack = document.getElementById("opponent-hp-track");
...
const playerTeamSlots = document.getElementById("player-team-slots");
const opponentTeamSlots = document.getElementById(
    "opponent-team-slots"
);
...
function renderPokemonStatus(pokemon, side) {
    ...
    const nameElement = isPlayer ? playerName : opponentName;
    const levelElement = isPlayer ? playerLevel : opponentLevel;
    const typeElement = isPlayer ? playerType : opponentType;
    const hpTrackElement = isPlayer
        ? playerHPTrack
        : opponentHPTrack;
    const hpBarElement = isPlayer ? playerHPBar : opponentHPBar;
    const hpTextElement = isPlayer
        ? playerHPText
        : opponentHPText;
    ...
    typeElement.textContent = formatTypeName(pokemon.type);
    removeTypeClasses(typeElement);
    typeElement.classList.add(`type-${pokemon.type}`);
...
function createMoveButton(move, index) {
    ...
    button.className = `move-button type-${move.type}`;
    button.type = "button";
    ...
    button.append(moveName, moveDetails);
...
function renderMoveButtons(pokemon) {
    ...
    pokemon.moves.forEach((move, index) => {
        const moveButton = createMoveButton(move, index);
        moveGrid.appendChild(moveButton);
    });
}

function createTeamSlot(pokemon, activePokemon) {
...
function renderTeamSlots(
    ...
    team.forEach((pokemon) => {
        const slot = createTeamSlot(pokemon, activePokemon);
        containerElement.appendChild(slot);
    });
}

function renderBattleScreen() {
    renderPokemonStatus(activePlayerPokemon, "player");
    renderPokemonStatus(activeOpponentPokemon, "opponent");

    renderMoveButtons(activePlayerPokemon);
    ...
    battleMessage.textContent =
        `What will ${activePlayerPokemon.name} do?`;
}
...
function resetBattle() {
    ...
    console.log("Pocket Clash battle data reset.");
}

function initializeGame() {
    renderBattleScreen();

    console.log("Pocket Clash data models initialized.");
    console.log("Available Pokemon:", POKEMON_DATABASE);
    console.log(
        "Fire against Grass:",
        getTypeEffectiveness("fire", "grass")
    );
}
